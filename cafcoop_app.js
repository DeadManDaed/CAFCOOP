/**
 * cafcoop_app.js - VERSION COMPLÈTE FINALE
...
window.chargerPhoto = (input) => {
    if (input.files && input.files[0]) {
        ...
        reader.onload = (e) => {
            ...
            document.getElementById('apercu-photo').innerHTML = `<img src="${e.target.result}" style="width:100px; border-radius:8px; margin-top:10px;">`;
            verifierBoutonEnvoi();
        };
...
window.transfererExpert = async (id) => {
    try {
        await supabase.from('diagnostics').update({ statut: 'transfere' }).eq('id_diagnostic', id);
...
window.marquerLivree = async (id) => {
    try {
        await supabase.from('commandes').update({ statut: 'livree' }).eq('id_commande', id);
